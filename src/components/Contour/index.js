...
export default class Contour extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            //矩阵数据 
    ...
    componentDidMount() {
        if (this.props.data) {
            const { rowNum, columnNum, min, max, dataSource } = this.props.data[0];
    ...
    importData(e) {
        ...
        if (e.target.value) {
            ...
            reader.onload = function () {
                ...
                let dataArray = [];
                //将文件内容去除两端空格后逐行分割
                let columns = importJson1.trim().split('\n');
                //将以#开头的那行字符串去除，并将剩余字符串逐行push进dataArray
                ...
                for (let i = 0; i < rowNum; i++) {
                    //将dataArray中的每一项出去两端空格后再按空格分割
                    let a = dataArray[i].trim().split(' ');
                    columnNum = a.length;
                    //b为最终二维矩阵中的每个数组
                    let b = [];
                ...
                //获取二维数组中的最大值和最小值
                max = min = dataSource[0][0];//先把dataSource[0][0]赋值给max和min 
                for (let i = 0; i < rowNum; i++) {//依次判断二维数组中的每一个数 
                    for (let j = 0; j < columnNum; j++) {
                        if (max < dataSource[i][j]) max = dataSource[i][j];//如果该数大于max，就把其赋值给max 
    ...
    //echarts显示部分
    showEcharts(rowNum, columnNum, min, max, dataSource) {
        ...
        function generateData(theta, min, max) {
            ...
            for (let i = 0; i < rowNum; i++) {
                for (let j = 0; j < columnNum; j++) {
                    ...
                    data.push([j, rowNum - i - 1, dataSource[i][j]]);
    ...
    render() {
        ...
        return (
            <div style={{ margin: "20px", height: "100%" }}>
                {data ?
                    <>
                        <label style={{ marginRight: "10px" }}>请选择结果:</label>
                        <Select defaultValue={data[0].name} style={{ width: 120 }} onChange={this.handleChange.bind(this)}>
                            {data.map((echartsData, index) => {
                                return (
                                    <Option key={index}>
                                        {echartsData.name}
                                    </Option>
