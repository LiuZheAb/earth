...
export default class csvView extends Component {
    ...
    result = () => {
        ...
        let SCALARS = [], xAxis = [], yAxis = [];
        Rendering(model, this.container);
        for (let i = 0; i < yLength; i++) {
            yAxis.push(i)
            for (let j = 0; j < xLength; j++) {
                SCALARS.push(data[i][j]);
            }
        }
    ...
    render() {
        ...
        let { moveStyle, screen, ruler, attribute, scale, ranging, theme } = state;
        let scaleOpc = 0;
        if (scale === true) {
            scaleOpc = 1;
        } else {
        ...
        if (model.renderer) {
            ...
            showBoundRuler(ruler, model, this.container, vtk(model.actor.getMapper().getInputData().getState()), this.props, dimensional, fontColor, model.xAxis, model.yAxis, "textCanvas"); //刻度标尺
        }
        ...
        return (
            <div>
                            ...
                            <InputGroup>
                                ...
                                <Row >
                                    <Col >
                                        ...
                                        <Input className="hueRangemax" type="text" style={{ width: "50%" }} defaultValue="1" />
                                    </Col >
                                </Row>
                                <Row >
                                ...
                                <Row >
                                    <Col >
                                        <Input className="valueRangemin" type="text" style={{ width: "50%" }} defaultValue="-1" />
                ...
                <div style={{ width: "8%", minWidth: "100px", position: "absolute", left: "5%", bottom: "5%", opacity: scaleOpc, borderBottom: "2px solid " + fontColor }}>
                    <div style={{ height: "1vh", position: "relative" }}>
                        <div style={{ position: "absolute", left: "0", borderLeft: "2px solid " + fontColor, height: '100%' }}>
                            <span style={{ position: "absolute", color: fontColor, display: "inline-block", bottom: "100%", transform: "translateX(-50%)" }}>0</span>
                        </div>
                        <div style={{ position: "absolute", left: "50%", borderLeft: "2px solid " + fontColor, height: '100%' }}>
                            <span style={{ position: "absolute", color: fontColor, display: "inline-block", bottom: "100%", transform: "translateX(-50%)" }}>50</span>
                        </div>
                        <div style={{ position: "absolute", right: "0", borderRight: "2px solid " + fontColor, height: '100%' }}>
                            <span style={{ position: "absolute", color: fontColor, display: "inline-block", bottom: "100%", transform: "translateX(-50%)" }}>100</span>
                        </div>
                    </div>
                </div>
            </div>
