...
class Details extends React.Component {
    ...
    startDocker = () => {
        ...
        // this.setState({ loading: false, status: 5 });
        axios({
            method: 'post',
            url: api + 'runContain',
            data: {
    ...
    createPFile = e => {
        e.preventDefault();
        let { texts, selects, radios, checkBoxs, textAreas, inputFiles, idenMod, uploadBoxs, dockerID, dockerIP, vport, currentStep, moduleName, currentStep2, idenMod2, proIndex, status, baseUrl, calcApi, requestMethod } = this.state;
        let _this = this;
        if (status === 5) {
            if (calcApi) {
                if (checkNullvalue(texts) && checkNullvalue(selects) && checkNullvalue(radios) && checkNullvalue(checkBoxs) && checkNullvalue(textAreas)) {
                    let baseUrl = "http://139.217.82.132:5050";
    ...
    render() {
        ...
        const { username, appName, texts, selects, radios, checkBoxs, textAreas, uploadBoxs, inputFiles, loading, listener, currentStep,
            resultData, staticURL, isComputing, idenMod, dockerID, dockerIP, vport, logInfoArray,
