...
import { Layout, Button, Menu, Select, Radio, Checkbox, Input, message, Card, Col, Row, Form, Upload, Icon, Empty, Result, Steps } from 'antd';
...
class Details extends React.Component {
    ...
    createPFile = () => {
        let { textBoxs, selectBoxs, radioBoxs, checkBoxs, textAreas, uploadBoxs } = this.state;
        let _this = this;
        for (let i = 0, len = textBoxs.length; i < len; i++) {
            if (textBoxs[i].paramName === "iter_max" || textBoxs[i].paramName === "time_after_earthquake") {
                textBoxs[i].currentValue = "+" + textBoxs[i].currentValue;
            }
        }
        if (checkNullvalue(textBoxs) && checkNullvalue(selectBoxs) && checkNullvalue(radioBoxs) && checkNullvalue(checkBoxs) && checkNullvalue(textAreas)) {
            let makeJson = `{ "texts" :${JSON.stringify(textBoxs)}
    ...
    render() {
        ...
        const { appName, collapsed, textBoxs, selectBoxs, radioBoxs, checkBoxs, textAreas, uploadBoxs, loading, content, listener, currentStep } = this.state;
        const uploadProps = {
            name: "uploadfile",
            action: api + "upload",
            headers: {
                        ...
                        <Col sm={8} xs={24} className="details-card">
                            <Card title="参数数据" bordered={false} className="params-card">
                                <Steps current={currentStep}>
                                    ...
                                    <Step title="上传参数文件"></Step>
                                    <Step title="应用服务计算"></Step>
                                </Steps>
                                <div>
                                    {currentStep === 0 &&
                                        <Button type="primary" className="" htmlType="submit" loading={loading} onClick={this.startDocker}>启动容器</Button>
                                    }
                                    {currentStep === 1 &&
                                        <Form {...formItemLayout} onSubmit={this.handleSubmit} className="details-form">
                                            ...
                                            <Row className="app-button">
                                                ...
                                                <Button type="primary" className="" onClick={this.createPFile}>创建</Button>
                                            </Row>
                                        </Form>
                                    }
                                    {currentStep === 2 &&
                                        <Button type="primary" className="" htmlType="submit" loading={loading}>应用服务计算</Button>
                                    }
                                </div>
