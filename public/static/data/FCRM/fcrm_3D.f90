	...
	!-------------------------------------------------------------------------
	!
	! MODULE gravity_magnetic_2D_cross_gradient_inversion
	!
    !-------------------------------------------------------------------------
    !
    !   Funtion£ gravity_magnetic_2D_cross_gradient_joint_inversion
    !   Data£º  10/2020
    !   Author£ºSheng Liu
    !
    !--------------------------------------------------------------------------
	MODULE fcrm_3d_gminv
    ...
    subroutine fcrm_3d_inversion(&
		  &z0,B0,tz,tx,az1,ax1,cengshu,x_number,y_number,iteration_max,&
		  &zzmax,b_low_g,b_up_g,b_low_m,b_up_m,&
		  &x_n_coordinate,y_n_coordinate,&
		  &x_coordinate,y_coordinate,m1,m2,mref_g,mref_m,&
		  &d_obs_g,d_obs_m,x,y,z,&
    ...
    double precision::xmin,xmax,zmin,zmax,ymin,ymax
    double precision,allocatable::x_source(:,:),y_source(:,:),z_source(:,:)
    !smooth inversion
    double precision,allocatable::wm_m(:,:),w_m(:),wd_m(:),mpri_m(:)
!    double precision::,allocatable::ww_m(:)
    double precision::ww_m
    double precision::as_m,ax_m,ay_m,az_m,lu_m
    double precision,allocatable::wm_g(:,:),w_g(:),wd_g(:),mpri_g(:)
!    double precision::,allocatable::ww_g(:)
    double precision::ww_g
    double precision::as_g,ax_g,ay_g,az_g,lu_g
    integer i,j
    !gaussian voice
    double precision,allocatable::a_gauss_g(:),a_gauss_m(:)
    double precision::normal,mean,sigma,eps
    !inducing_information
    integer layer
    double precision::PI=3.141592653
!    !local constrain
!    double precision::,allocatable::ws_m(:),ws_g(:)
    ...
    integer n_coordinate,cengshu,x_number,y_number
    integer n_source,iteration_max
	integer x_n_coordinate,y_n_coordinate
    double precision::z0,zzmax
    double precision::lam_g,lam_m,eta_g,eta_m
	double precision::b_low_g,b_up_g,b_low_m,b_up_m
    double precision::x_coordinate(n_coordinate),y_coordinate(n_coordinate)
	...
	double precision::x(n_source),y(n_source),z(n_source)
	integer C
...
!f2py integer, intent(in) :: n_coordinate,cengshu,x_number,y_number
!f2py integer, intent(in) :: n_source,iteration_max
!f2py integer, intent(in) :: x_n_coordinate,y_n_coordinate
...
!f2py double precision, intent(in) ::  x_coordinate,y_coordinate
...
!f2py double precision, intent(in,out) :: x,y,z
!f2py integer, intent(in) :: C
!f2py double precision, intent(in) ::  V1,tk1



	write(*,*)'GM 3D FCRM inversion.'
    write(*,*) '-------------------------------------------------------------';
    ...
    ymin=minval(y_coordinate);
    ymax=maxval(y_coordinate);
    tz=tz/180.0*PI;
    tx=tx/180.0*PI;
    az1=az1/180.0*PI;
    ax1=ax1/180.0*PI;
	...
	call input_coordinate_3d_vertical_reg(&
		&z_coordinate,x_n_coordinate,y_n_coordinate,&
		&z0,n_coordinate);
	...
	allocate( poufen_parame(cengshu,2) );
	call read_n_source(&
		&cengshu,x_number,y_number,poufen_parame,n_source);
	allocate(x_source(n_source,2),y_source(n_source,2),z_source(n_source,2))
	call read_source(&
	    &xmax,xmin,ymax,ymin,cengshu,poufen_parame,&
	    &x_source,y_source,n_source);
	call read_z_source(&
    ...
    call length_scale(&
        &xmin,xmax,ymin,ymax,as_g,ax_g,ay_g,az_g,poufen_parame,&
		&cengshu,zzmax,zmax);
    call length_scale(&
        &xmin,xmax,ymin,ymax,as_m,ax_m,ay_m,az_m,poufen_parame,&
		&cengshu,zzmax,zmax);
...
!	call CG_triple_reweight_3D_g(&
!		&m1,mref_g,d_obs_g,wd_g,w_g,lu_g,&
!		&cengshu,poufen_parame,as_g,ax_g,ay_g,az_g,ws_g,ww_g,&
!		&n_source,n_coordinate,x_source,y_source,z_source,&
!		&x_coordinate,y_coordinate,z_coordinate,b_low_g,b_up_g,iteration_max);
!
!
!	call CG_triple_reweight_3D_m(&
!		&m2,mref_m,d_obs_m,wd_m,w_m,lu_m,&
!		&cengshu,poufen_parame,as_m,ax_m,ay_m,az_m,ws_m,ww_m,&
!		&n_source,n_coordinate,x_source,y_source,z_source,&
...
!		&tx,tz,ax1,az1,B0,b_low_m,b_up_m,iteration_max);
    ...
    call fcrm_triple_3D_joint_inversion(&
        &m1,mref_g,d_obs_g,wd_g,w_g,lu_g,&
        &m2,mref_m,d_obs_m,wd_m,w_m,lu_m,&
        &cengshu,poufen_parame,&
        &as_g,ax_g,ay_g,az_g,ws_g,ww_g,&
        &as_m,ax_m,ay_m,az_m,ws_m,ww_m,&
        &n_source,n_coordinate,&
        &x_source,y_source,z_source,&
        &x_coordinate,y_coordinate,&
        &z_coordinate,tx,tz,ax1,az1,B0,lam_g,lam_m,eta_g,eta_m,&
        &b_low_g,b_up_g,b_low_m,b_up_m,iteration_max,C,V,tk);


	do i=1,n_source,1
		x(i)=(x_source(i,1)+x_source(i,2))/2.0;
		y(i)=(y_source(i,1)+y_source(i,2))/2.0;
    ...
    deallocate(wd_g,w_g);
    deallocate(wd_m,w_m);
    deallocate(x_source,y_source,z_source,poufen_parame);
    deallocate(z_coordinate);
	deallocate(V,tk);


    return
    End subroutine fcrm_3d_inversion
    ...
    !	 输出:x_coordinate,y_coordinate,z_coordinate
    !
    !----------------------------------------------------
    Subroutine input_coordinate_3d_vertical_reg(&
           &z_coordinate,x_number,y_number,z0,n_coordinate)


    integer i,j,xy,k,n_coordinate
    double precision::z_coordinate(n_coordinate)
    double precision::z0
    integer x_number,y_number
    integer m,n
    ...
    do i=1,n,1
        xy=(i-1)*m
        do j=1,m,1
            z_coordinate(j+xy)=z0;
        enddo
    enddo


    End	 subroutine input_coordinate_3d_vertical_reg
    ...
    Subroutine read_n_source(&
           &cengshu,x_n,y_n,poufen_parame,n_source)
    implicit none


    integer cengshu,n_source,k,x_n,y_n
    integer poufen_parame(cengshu,2)


    n_source=0;
    do k=1,cengshu,1
        poufen_parame(k,1)=x_n;
        poufen_parame(k,2)=y_n;
        n_source=n_source+poufen_parame(k,1)*poufen_parame(k,2);
    enddo
    ...
    !---------------------------------------------------------------------------
    !
    !  功能：计算分层后各个场源的坐标值(网格剖分部分没有问题了)，针对的是不考虑地形的问题
    !  输入：types:1(平面部分)
    !  输出：
    !
    !---------------------------------------------------------------------------
    ...
    integer poufen_parame(cengshu,2)
    double precision::x_source(n_source,2),y_source(n_source,2)
    ...
    n21=0;
    do k=1,cengshu,1
    n2=n2+poufen_parame(k,1)*poufen_parame(k,2)
    do i=n1,n2,1
        j=poufen_parame(k,2)
        l=poufen_parame(k,1)

        if(mod(i,l).eq.0)then     !每一层的每一行的最后一个网格
            x_source(i,2)=xmax
        ...
        if(mod(i,l).eq.0)then     !每一层的每一行的最后一个网格
            y_source(i,1)=ymin+(ymax-ymin)/j*((i-n21-mod(i-n21,l))/l-1)
            y_source(i,2)=y_source(i,1)+(ymax-ymin)/j
        endif
        if(mod(i,l).ne.0)then     !每一层的每一行的最后一个网格
            y_source(i,1)=ymin+(ymax-ymin)/j*((i-n21-mod(i-n21,l))/l)
            y_source(i,2)=y_source(i,1)+(ymax-ymin)/j
        endif
    enddo
    n1=n2+1;
    ...
    integer poufen_parame(cengshu,2)
    double precision::zmax,zzmax,l,z0
    ...
    n2=1;
    do i=1,cengshu,1
        n1=n1+poufen_parame(i,1)*poufen_parame(i,2)
        do j=n2,n1,1
    ...
    !
    !--------------------------------------------------------------------
    Subroutine depth_weighted_li_g( wz,n_source,z_source )
    implicit none
    ...
    !  输入：xmin,xmax,ymin,ymax,poufen_parame,cengshu
    !  输出：as,ax,ay,az
    !
    !---------------------------------------------------------------------------
    subroutine length_scale(xmin,xmax,ymin,ymax,&
           &as,ax,ay,az,poufen_parame,cengshu,zzmax,zmax)

    implicit none

    double precision::xmin,xmax,ymin,ymax,ax,ay,as,az,d_x,d_y,d_z,zzmax,zmax
    integer cengshu,poufen_parame(cengshu,2)


    d_x=(xmax-xmin)/poufen_parame(1,1) ;         !均匀剖分，下层剖分的情况于第一层一致的
    d_y=(ymax-ymin)/poufen_parame(1,2);
...
!    ax=(2*d_x)**2*as*1.0;
    ...
    !--------------------------------------------------------------------------
    !
    !  功能：计算wx,wy,wz,ws权重系数（有无先验信息时候再具体采用不同的方法再进行计算,
    ...
    !
    !  功 能：计算模型约束项的梯度
    !  输 入：as,ax,ay,az,ww,w
    !  输 出：Wm*Wm*m
    !   Wm(n,n) = [as*ws101*ds*w(n)]^2 + [ax*dx(n,n)*w(n)]^2 + ……
    !---------------------------------------------------------------------------
    Subroutine computation_WWm(&
        &x_source,y_source,z_source,n_source,cengshu,&
        &poufen_parame,as,ax,ay,az,ws101,ww,&
        &n_coordinate,w,m,wwm )
    implicit none


    integer n_coordinate,n_source,i,j,k,kk,l,l1,k1,cengshu
    integer poufen_parame(cengshu,2)
    integer nx,ny,nz
    double precision::x_source(n_source,2),y_source(n_source,2),&
           &z_source(n_source,2)
    double precision::d_x,d_y,d_z,ds
    double precision,allocatable::dx(:,:),dy(:,:),dz(:,:)
    double precision::ax,as,ay,az
    double precision::w(n_source),ww,ws101
    double precision::m(n_source),wwm(n_source)
    double precision,allocatable::ws(:),wx(:,:),wy(:,:),wz(:,:)
    double precision,allocatable::ws2(:),wx1(:),wy1(:),wz1(:),&
           &wx2(:),wy2(:),wz2(:)


    !均匀剖分的情况
    d_x=x_source(1,2)-x_source(1,1);
    d_y=y_source(1,2)-y_source(1,1);
    d_z=z_source(1,2)-z_source(1,1);


    !计算ds
    ds=1.0*sqrt( d_x*d_y*d_z );
    !计算Ws=α*D*Z：as*ds*wz
    ...
    !-----------------------------------------------------------
    !计算dx
    nx=(poufen_parame(1,1)-1)*poufen_parame(1,2)*cengshu*2;
    allocate( dx(nx,3) );
    dx=0.0;
    k=0;
    do i=1,n_source,1
        if( ( mod( i,poufen_parame(1,1) ) ).ne.0 )then
            k=k+1;
...
!    !$omp end parallel
    ...
    !-----------------------------------------------------------
    !计算dy
    ny=poufen_parame(1,1)*(poufen_parame(1,2)-1)*cengshu*2;
    allocate( dy(ny,3) );
    dy=0.0;
    k=0;
    kk=poufen_parame(1,1)*poufen_parame(1,2);
    do i=1,n_source,1
        if( ( i-int((i-1)/kk)*kk ).le.( kk-poufen_parame(1,1) ) )then
            k=k+1;
            dy(k,1)=i;  dy(k,2)=i;  dy(k,3)=-1;
            k=k+1;
            dy(k,1)=i;  dy(k,2)=i+poufen_parame(1,1);  dy(k,3)=1;
        endif
    enddo
...
!    !$omp end parallel
    ...
    !计算梯度中间量：Wy1=Wy*m
    allocate( wy1(n_source) );
    wy1=0.0;
!    !$omp parallel
!    !$omp do private(i) schedule(guided)
    do i=1,ny-1,2
        j=wy(i,1);  k=wy(i,2);  l=wy(i+1,2);
        wy1(j) = wy(i,3)*m(k) + wy(i+1,3)*m(l);
    end do
!    !$omp end do
!    !$omp end parallel


    !计算梯度：Wy2=Wy'*(Wy*m)=Wy'*Wy1
    allocate( wy2(n_source) );
    wy2=0.0;
!    !$omp parallel
!    !$omp do private(j) schedule(guided)
    do j=1,n_source,1
...
!    !$omp end parallel
    ...
    !-----------------------------------------------------------
    !计算dz
    nz=poufen_parame(1,1)*poufen_parame(1,2)*(cengshu-1)*2;
    allocate( dz(nz,3) );
    dz=0.0;
    k=0;
    kk=poufen_parame(1,1)*poufen_parame(1,2);
...
!    !$omp end parallel
    ...
    !
    !  功 能：计算模型约束项
    !  输 入：as,ax,ay,az,ws,ww,wz,(m1-mref)
    ...
    double precision,allocatable::ws(:),wx(:,:),wy(:,:),wz(:,:)
    double precision,allocatable::ws1(:),wx1(:),wy1(:),wz1(:)
    double precision::ws2,wx2,wy2,wz2
...
!    !$omp end parallel
    ...
    !计算Wy2=(Wy*m)'*Wy*m=Wy1'*Wy1
    wy2=0.0;
    !$acc parallel
    !$acc loop reduction(+:wy2)
    do i=1,n_source,1
        wy2=wy2+wy1(i)*wy1(i);
    enddo
    !$acc end loop
...
!    !$omp end parallel
    ...
    !
    !   功能：重力核函数
    !	输入:n_source,x_source,y_source,z_source,m,ax,az,tx,tz,
    !        n_coordinate,x_coordinate,y_coordinate,z_coordinate
    !	输出:g
    !
    !--------------------------------------------------------------------------
    subroutine computation_G_g( g1,&
		   &n_coordinate,n_source,x_source,y_source,z_source,&
		   &x_coordinate,y_coordinate,z_coordinate )
    implicit none
    ...
    double precision::g1(1:n_coordinate,1:n_source)
    double precision::x_source(n_source,2),y_source(n_source,2),&
           &z_source(n_source,2)
    double precision::x_coordinate(n_coordinate),y_coordinate(n_coordinate),&
    ...
    g1=0.0
    !$acc data
    !$acc kernels loop copy(g1) private(x12,y12,z12,g2,x,y,z,r,a,delt_g)
    do k=1,n_coordinate,1
      do n=1,n_source,1
        do i=1,2,1
          do j=1,2,1
            do l=1,2,1
              x12=x_source(n,i);y12=y_source(n,j);z12=z_source(n,l)
              x=x_coordinate(k);y=y_coordinate(k);z=z_coordinate(k)


              r=sqrt((x12-x)**2+(y12-y)**2+(z12-z)**2)


              delt_g=-g2*((x12-x)*(log(y12-y+r))+(y12-y)*(log(x12-x+r))-&
                &(z12-z)*atan2((x12-x)*(y12-y),(z12-z)*r))


              a=delt_g


              g1(k,n)=g1(k,n)+((-1)**(i+j+l))*a
            enddo
         enddo
       enddo
     enddo
    ...
    return
    end	subroutine computation_G_g


    !--------------------------------------------------------------------------
    !   行核函数：n_source
    !--------------------------------------------------------------------------
    subroutine computation_G_g_Row( g1,&
           &n_source,x_source,y_source,z_source,x,y,z )
    implicit none


    integer k,n,i,j,l
    integer n_source
    double precision::g1(1:n_source)
    double precision::x_source(n_source,2),y_source(n_source,2),&
		   &z_source(n_source,2)
    ...
    !   列核函数：n_coordinate
    !--------------------------------------------------------------------------
    subroutine computation_G_g_Column( g1,&
          &n_coordinate,n_source,n,x_source,y_source,z_source,&
          &x_coordinate,y_coordinate,z_coordinate )
    implicit none
    ...
    double precision::g1(1:n_coordinate)
    double precision::x_source(n_source,2),y_source(n_source,2),&
           &z_source(n_source,2)
    double precision:: x_coordinate(n_coordinate),y_coordinate(n_coordinate),&
           &z_coordinate(n_coordinate)
...
!     enddo
    ...
    !   行核函数：n_source
    !----------------------------------------------------
    Subroutine computation_G_mag_Row(g,&
           &n_source,x_source,y_source,z_source,&
           &x,y,z,B0,tz,tx,az,ax)
    implicit none

    integer n_source
    double precision::x_source(n_source,2),y_source(n_source,2),&
           &z_source(n_source,2)
    double precision::x,y,z,x12,y12,z12,r
    double precision::G(n_source)
    double precision::tx,tz,ax,az,B0
    integer k,n,i,j,l
    double precision::k1,k2,k3,k4,k5,k6,dx0,dy0,dz0,dx,dy,dz


!    dx0=cosd(tz)*cosd(tx); dy0=cosd(tz)*sind(tx); dz0=sind(tz);
!    dx =cosd(az)*cosd(ax); dy =cosd(az)*sind(ax); dz =sind(az);
    dx0=cos(tz)*cos(tx); dy0=cos(tz)*sin(tx); dz0=sin(tz);
    dx =cos(az)*cos(ax); dy =cos(az)*sin(ax); dz =sin(az);
    k1=-2*dx0*dx;     k2=-2*dy0*dy;     k3=2*dz0*dz;
    k4=dz*dy0+dz0*dy; k5=dz*dx0+dx*dz0; k6=dy*dx0+dx*dy0;
    G=0.0;
       ...
       do n=1,n_source,1
          do i=1,2,1
             do j=1,2,1
                ...
                G(n)=G(n)+( (-1)**(i+j+l) )*( &
                & k1*( atan2( (x12-x),(y12-y)+(z12-z)+r ) )+&
                & k2*( atan2( (y12-y),(x12-x)+r+(z12-z) ) )+&
                & k3*( atan2( (x12-x)+(y12-y)+r,(z12-z) ) )+&
                & k4*( log(r+(x12-x)) )+&
                & k5*( log(r+(y12-y)) )+&
                & k6*( log(r+(z12-z)) ) )*100;
...
!    enddo
    ...
    !   列核函数：n_coordinate
    !----------------------------------------------------
    Subroutine computation_G_mag_Column(g,&
           &n_coordinate,n_source,n,x_source,y_source,z_source,&
           &x_coordinate,y_coordinate,z_coordinate,B0,tz,tx,az,ax)
    implicit none
...
!      enddo
    ...
    !
    !----------------------------------------------------------------------
    Subroutine CG_triple_reweight_3D_g( &
        &m1,mref,d_obs,wd,w,lu,&
        &cengshu,poufen_parame,as,ax,ay,az,ws,ww,&
        &n_source,n_coordinate,x_source,y_source,z_source,&
        &x_coordinate,y_coordinate,z_coordinate,&
        &b_low_g,b_up_g,iteration_max )
    implicit none
    ...
    double precision::m1(n_source),m1_w(n_source)
    double precision::mref(n_source),mref_w(n_source)
    ...
    double precision::b_low_g,b_up_g
...
!    !m1越小，beta值越大，模型下移
    ...
    call computation_module_valaue( &
        &x_source,y_source,z_source,n_source,cengshu,&
        &poufen_parame,as,ax,ay,az,ws,ww,&
        &n_coordinate,w,(m1-mref),fm );
    ...
    call  cpu_time(t2);
    write(*,*) 'The time required for joint inversion.';
    write(*,*)  't2-t1=',t2-t1,'s';
    write(*,*) '-------------------------------------------------------------';


    return
    End subroutine CG_triple_reweight_3D_g
    ...
    !
    !----------------------------------------------------------------------
    Subroutine CG_triple_reweight_3D_m( &
		   &m1,mref,d_obs,wd,w,lu,&
		   &cengshu,poufen_parame,as,ax,ay,az,ws,ww,&
		   &n_source,n_coordinate,x_source,y_source,z_source,&
		   &x_coordinate,y_coordinate,z_coordinate,&
    ...
    double precision::b_low_m,b_up_m
    double precision::t1,t2
...
!    !m1越小，beta值越大，模型下移
    ...
    call computation_gradient_re_m( &
        ...
        &cengshu,poufen_parame,as,ax,ay,az,ws,ww,&
        &n_coordinate,n_source,x_source,y_source,z_source,&
        &x_coordinate,y_coordinate,z_coordinate,B0,tz,tx,az1,ax1 );
    ...
    do while( (pl1.ge.2.0e-1).and.(kk.lt.iteration_max) )
        ...
        call update_search_d( n_source,d,f0,f );
        ...
        call Anti_reweighting_m( n_source,m1_w,w,m1 );
    ...
    return
    End subroutine CG_triple_reweight_3D_m


    !----------------------------------------------------------------------
    !
    !  功能：计算数据目标函数的梯度
    !  f= G_w'*Wd'*Wd*(G_w*m1_w - d_obs) + β*Wm_w'*Wm_w*(m1_w - mref_w)
    !   = (w-1)'*[ G'*Wd'*Wd*(G*m1 - d_obs) ] + β*(w-1)'*[ Wm'*Wm*(m1 - mref) ]
    !
    !------------------------------------------------------------------------
    ...
    do j=1,n_source,1
        g2=0.0;
        call computation_G_g_Column( g2,&
            &n_coordinate,n_source,j,x_source,y_source,z_source,&
            &x_coordinate,y_coordinate,z_coordinate );
    ...
    !重加权计算：(w-1)'*[Wm'*Wm*(m1 - mref)]
    allocate( g12_w(n_source) )
    g12_w=0.0;
    do i=1,n_source,1
        g12_w(i) = g12(i)/w(i);
    enddo
    ...
    !
    !------------------------------------------------------------------------
    Subroutine update_search_d(n_source,d,f0,f)
    implicit none
    ...
    !----------------------------------------------------------------------
    !
    !  功能：计算步长计算中间量：分母dk'*A_w*dk= dk'*A_w*dk
    ...
    allocate( d_pre(n_coordinate) )
    d_pre=0.0;
    call computation_d_pre_g( d_pre,m1,&
        &n_coordinate,n_source,x_source,y_source,z_source,&
        &x_coordinate,y_coordinate,z_coordinate );
    ...
    allocate( g2(1:n_coordinate),g11(n_source) )
    g11=0.0;
    do j=1,n_source,1
        g2=0.0;
        call    computation_G_mag_Column( g2,&
            &n_coordinate,n_source,j,x_source,y_source,z_source,&
            &x_coordinate,y_coordinate,z_coordinate,B0,tz,tx,az1,ax1 );
    ...
    ! b = dk'*A_w*dk = dk'*(G_w'*Wd'*[ Wd*G_w)*dk ] + β*dk'*(Wm_w'*[ Wm_w)*dk ]
    !
    !------------------------------------------------------------------------
    ...
    do i=1,n_coordinate,1
        ...
        z=z_coordinate(i);
        g1=0.0;
        call computation_G_mag_Row( g1,&
            &n_source,x_source,y_source,z_source,x,y,z,B0,tz,tx,az1,ax1 );
    ...
    !
    !------------------------------------------------------------------------
    Subroutine  fcrm_triple_3D_joint_inversion( &
           ...
           &b_low_g,b_up_g,b_low_m,b_up_m,iteration_max_joint,C,V,tk)
    implicit none
    ...
    integer C,cengshu
    integer poufen_parame(cengshu,2)
    double precision::ax_g,as_g,ay_g,az_g,ww_g,ws_g
    double precision::ax_m,as_m,ay_m,az_m,ww_m,ws_m
    double precision::x_source(n_source,2),y_source(n_source,2),&
           &z_source(n_source,2)
    double precision::x_coordinate(n_coordinate),y_coordinate(n_coordinate),&
           &z_coordinate(n_coordinate)
    ...
    call computation_module_valaue( &
        &x_source,y_source,z_source,n_source,cengshu,&
        &poufen_parame,as_g,ax_g,ay_g,az_g,ws_g,ww_g,&
    ...
    call computation_module_valaue( &
        &x_source,y_source,z_source,n_source,cengshu,&
        &poufen_parame,as_m,ax_m,ay_m,az_m,ws_m,ww_m,&
    ...
    do while( ((pl1.ge.eps).or.(pl2.ge.eps)).and.(kk.lt.iteration_max_joint) )
            ...
            !call    update_guide_weighting_parameter( V1,V2,C,tk,eta_g,eta_m );
    ...
    call cpu_time(t20);
    write(*,*)t20-t10,'Inversion time(s)=','s';



    return
    End subroutine fcrm_triple_3D_joint_inversion
